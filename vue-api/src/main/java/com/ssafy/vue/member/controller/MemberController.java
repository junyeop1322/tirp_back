...
public class MemberController {
	...
	public ResponseEntity<?> idChk(@PathVariable String userId){
		log.debug("check id : {}", userId);
		Map<String, Object> resultMap = new HashMap<>();
		HttpStatus status = HttpStatus.OK;
		resultMap.put("checkResult", true);
		try{
			if(!memberService.idDuplicatedCheck(userId)){
...
//				발급받은 refresh token을 DB에 저장.
				memberService.saveToken(loginUser.getUserId(), accessToken);
...
//				로그인 사용자 정보.
	...
	public ResponseEntity<?> removeToken(@PathVariable ("userId") @ApiParam(value = "로그아웃할 회원의 아이디.", required = true) String userId) {
		...
		HttpStatus status = HttpStatus.ACCEPTED;
		try {
			memberService.deleToken(userId);
			status = HttpStatus.OK;
		} catch (Exception e) {
			log.error("로그아웃 실패 : {}", e);
			resultMap.put("message", e.getMessage());
	...
	public ResponseEntity<?> refreshToken(@RequestBody MemberDto memberDto, HttpServletRequest request)
		...
		if (jwtUtil.checkToken(token)) {
			if (token.equals(memberService.getToken(memberDto.getUserId()))) {
